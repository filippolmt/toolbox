...
`internal/build.ResolveImage(image, registryMirror)` owns the ref precedence (full `image` override > `registry_mirror` host swap > canonical default); `imageplan.Ensure` requires the resolved image locally, `imageplan.RefreshAtStart` implements the `pull` policy at shell start (`never` → no registry round-trip, `always` → an unconditional pull, `auto` → probe, then ask — [the prompt](../configuration.md#the-start-up-refresh-prompt)), and `imageplan.Refresh` is its silent form, run by a [session reload](../session-reload.md) before it destroys anything (`auto` → the TTL-cached refresh, since a reload adopts what the store holds and the prefetch is what advanced it). The user-visible semantics — canonical default, `toolbox build` escape hatch, the three config keys, and the caveat that a full `image` override doesn't see local `toolbox build` output while a `registry_mirror` does — live in [configuration](../configuration.md#image-selection).
