...
- **`rust` is the one base image pinned by digest**, because it is the only `FROM` whose contents are compiled *into* a stage's `/out` rather than hosting the download of one: on arm64 `rtk-builder` runs `cargo install`, so a floating tag makes that stage's output stop being a function of `RTK_VERSION` — the same version and dependency graph on two toolchains give two different binaries. The pin is what closes that, and `pinDigests` for the package in `renovate.json` is what keeps it from becoming a frozen toolchain: Renovate re-resolves the digest for the same tag, so upstream rebuilds arrive as a digest PR. Don't pin the other bases by imitation — for them the base moving is [Archive Drift](../../CONTEXT.md#archive-drift), priced differently. → [Fetch Nondeterminism](../../CONTEXT.md#fetch-nondeterminism)
