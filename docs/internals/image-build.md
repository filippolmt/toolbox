...
- **Final-stage RUN layers (apt/pip/npm — can't fan out) are ordered rare→frequent** by measured Renovate cadence (≈6-month window: graphifyy 100 bumps, claude-code 95, wrangler 37, pnpm 36, codex 34, oci 20, codegraph 15, playwright-cli 10, cf 8, azure 7, playwright 7, pyright 5, typescript 2). Least-bumped first, most-bumped last, so a bump rebuilds only what sits below it: the daily graphify and claude-code bumps move 2-3 substantial layers where they used to move the whole tail. Two orderings inside that are load-bearing rather than cosmetic — `oci` stays above `graphifyy` so the two pip installs keep resolving their shared dependencies in the same order (the build verifies graphify *before* oci, so an inversion would pass green), and `playwright` stays above `playwright-cli`. Both fall out of the cadence order anyway. This ordering covers only bumps that land *in* the tail; the majority, which land in a fetch stage, are handled by the point above. Re-measure with `git log --since=… -p -- internal/build/assets/Dockerfile | grep '^+ARG'` before reshuffling.
