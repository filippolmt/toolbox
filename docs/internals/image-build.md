...
- **A stage that ships a git checkout runs `freeze-git` over each one first.** Freezing mtimes is enough for an unpacked release artefact, not for a repository: the pack is whatever the server's `upload-pack` chose to send and varies between responses, every reflog line carries a wall-clock timestamp, and the index caches the ctime and inode of every file the checkout wrote. `freeze-git` recompresses the pack locally (`git repack -adFqn` — **`-F`, not `-f`**: `-f` refuses the received *deltas* but still copies each non-delta object's compressed bytes verbatim, which is exactly how two builds of `fetch-brew` were measured producing packs with identical write order, identical delta bases and different sizes in pack), deletes `.git/logs`, and reads the index back from `HEAD` so its stat fields are zeroed. The entries are then stat-dirty, so the first `git status` in the container compares content rather than trusting the cache and reports the same clean tree. `fetch-omz` (the OMZ tree and each plugin) and `fetch-brew` are the two stages that need it, and neither can drop `.git` instead — Homebrew *is* a git checkout and `brew --version` reads it with `git describe`, and `~/.oh-my-zsh` needs one for `omz update`. It runs before the `chmod -R a+rwX`, which has to cover the pack and index it writes. → [Fetch Nondeterminism](../../CONTEXT.md#fetch-nondeterminism)
