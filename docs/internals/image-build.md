...
- **A stage that ships a git checkout runs `freeze-git` over each one first.** Freezing mtimes is enough for an unpacked release artefact, not for a repository: the pack is whatever the server's `upload-pack` chose to send and varies between responses, every reflog line carries a wall-clock timestamp, and the index caches the ctime and inode of every file the checkout wrote. `freeze-git` recompresses the pack locally (`git repack -adFqn` — **`-F`, not `-f`**, because `-f` refuses the received *deltas* but still copies each non-delta object's compressed bytes verbatim), deletes `.git/logs`, and reads the index back from `HEAD` so its stat fields are zeroed; the entries are then stat-dirty, so the first `git status` in the container compares content rather than trusting the cache and reports the same clean tree. It addresses the repository it is given by `--git-dir`/`--work-tree` rather than `-C`, which would *discover* one by walking up — and the OMZ plugins are checked out inside the OMZ working tree, so a plugin that failed to become a repository would otherwise get its parent normalised in its place. `fetch-omz` (the OMZ tree and each plugin) and `fetch-brew` are the two stages that need it, and neither can drop `.git` instead — Homebrew *is* a git checkout and `brew --version` reads it with `git describe`, and `~/.oh-my-zsh` needs one for `omz update`. It runs before the `chmod -R a+rwX`, which has to cover the pack and index it writes. → [Fetch Nondeterminism](../../CONTEXT.md#fetch-nondeterminism)
- **`rtk-builder` is knowingly not reproducible on arm64**, and it is the only `FROM` for which that is true, because it is the only one whose contents are compiled *into* a stage's `/out` rather than hosting the download of one: on arm64 the stage runs `cargo install`, so a floating base tag makes its output stop being a function of `RTK_VERSION`. A digest pin would close it, and was withdrawn — re-resolving the same tag costs a PR on every upstream rebuild, and the moved-layer gate measures amd64 only, where the binary is a checksummed tarball identical across builds. Don't pin any base by imitation either — for the others the base moving is [Archive Drift](../../CONTEXT.md#archive-drift), priced differently. → [Fetch Nondeterminism](../../CONTEXT.md#fetch-nondeterminism)
- **The final stage's RUN tail is empty of tool installs.** It used to hold thirteen of them, ordered rare→frequent by measured Renovate cadence so a bump rebuilt only what sat below it. Sorting can only ever make the *bottom* of a tail cheap, and it does nothing at all about Archive Drift — every tail RUN is parent-chained below the deliberately-unpinned base apt layer, so a Debian archive update moves the whole tail for nothing anyone edited. So every one of them moved into its own `fetch-*` stage instead, where a bump costs one `--link` layer whatever its position and a drift costs nothing. The npm ones must run the same node as the final stage — an npm global tree is only valid on the runtime that resolved it — which is why the node image is named once as the `node-base` stage and everything that needs it derives from that; `fetch-oci` installs into a venv at `/opt/oci-cli` and `fetch-graphify` uses `pip --root /out`, which keeps the system `dist-packages` layout the smoke test's `pip show` reads. What is left in the tail is `pnpm` — corepack materialises it into its own runtime-keyed cache rather than a prefix a COPY could carry — plus the apt layer and a handful of kB-sized setup RUNs. The two things that would force a tool back in: dpkg state (a `COPY --link` carries files, not packages; the base apt layer takes those, and a version-derived package set — playwright's chromium deps — needs a guard in `smoke-test.sh` before it can be frozen there, described under *Base apt layer* in the Dockerfile itself) and a RUN that reads a copied file, which drags that COPY back above the whole tail. Figures: ADR 0002 follow-up 2 and [ADR 0016](../adr/0016-image-size-two-denominators.md#the-third-denominator-which-this-adr-itself-missed).

  What made that move worth doing is that **each version ARG is declared immediately above the single RUN that consumes it**. An ARG in scope enters the cache key of every RUN below it — that is the `|N` prefix `docker history` shows per layer — so while all 14 were declared as one block at the top of the stage, all 21 tail RUNs were keyed on all 14 versions: any single bump rebuilt the entire tail and the position of a RUN bought nothing. The gate measured 16 layers above 1 MB and 694 MB on the transition that surfaced it — a transition carrying an `OCI_VERSION` bump plus an unrelated asset change, and billed by a gate that had not compared since the previous publish was cancelled, so read the figure as the scale of the churn rather than the price of one bump (ADR 0002, Follow-up). `TestFinalStageARGsScopedToTheirRUN` holds the placement, and scoping alone is not enough to clear the gate — see the follow-up.
...
Every npm fetch stage ends with `prune-node-weight <tree>…`, a helper `fetch-base` writes beside `freeze-mtimes` and each stage copies in. It strips weight from `node_modules`: source maps, `*.md` / `*.markdown`, `CHANGELOG*`, `AUTHORS*`, and `docs/`/`examples/`/`__tests__/`/`.github/` dirs. One helper rather than a `find` pasted into every stage, because the exclusion below is the kind of thing a ninth copy forgets.

**Gotcha — functional `.md` assets are collateral.** Some packages ship runtime-required content as `.md`. `playwright-cli` is the live example: its agent skills ship as `SKILL.md` + `references/*.md` under `playwright-core/lib/tools/skills/<name>/` (plus a `@playwright/cli/skills/` copy). The blanket `*.md` prune empties those, so `playwright-cli install --skills` then copies an **empty** skill — the failure looks like a working install (exit 0, dirs created) but no `SKILL.md`. `prune-node-weight` therefore leads with `-type d \( -name skills -o -name skill \) -prune -o …` — for every stage, not just playwright-cli's — and `fetch-playwright-cli` ends with a build-time `test -s …/lib/tools/skills/playwright-cli/SKILL.md` so a future prune/layout change that re-empties the source fails the build instead of shipping a silently-broken skill. `TestPruneNodeWeightSparesSkillTrees` reassembles the helper from the Dockerfile and runs it over a tree of this shape. The smoke test (`playwright-cli skill install`) re-checks it functionally at runtime. When adding a `.md`-pruning layer for a tool that bundles `.md` assets it actually uses, spare the asset dir the same way.
