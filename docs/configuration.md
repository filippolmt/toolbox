...
| [`agent`](#agent) | string | `claude` | Default AI agent auto-launched by [`toolbox worktree`](commands.md#toolbox-worktree): `claude`, `codex` or `pi`. |
...
| [`peer_messaging`](#peer_messaging) | bool | `false` | Let Claude Code sessions in different toolbox containers see and message each other. |
...
```
 main*  feat-xyz │  Opus 5 high FAST │ @reviewer │ vim:NORMAL │ ▰▱▱▱▱ 22% 1M │ ❄ │ 5h 24% 2h30m · 7d 41% 2d
```
...
Four things are absent on purpose, each because another surface already shows it **at the same moment**, which is what separates a duplicate from a second opinion. The open PR and the permission mode: Claude Code prints both on its own hint line, one row below. The repository name: herdr labels its workspace with it, in the sidebar. The working directory: starship prints it with the same truncation.

The branch stays, and the distinction is worth the words. starship prints it too, but only in the prompt — which has scrolled out of view by the time an agent has been working for a while, and which goes stale the moment that agent switches branch under you. herdr labels every worktree workspace of one repository identically, so its sidebar cannot tell them apart either.
...
One keypress is the whole answer — the terminal is in raw mode for the length of the question, so nothing waits on a Return behind it, and a Return typed out of habit behind the key is swallowed rather than handed on to the shell that is about to start. `ctrl+c` stops `toolbox shell` outright: it is not read as a "no", so no session starts and no postponement is recorded. `y`, a bare Return, or letting the countdown run out all download it and start the session on the new image. **`n` is "later", not "no"**: the session starts on the image already in your store while the [background prefetch](session-reload.md) fetches the new one anyway, so `toolbox-reload` moves you onto it whenever you want it. Nothing extra downloads on the "later" path — the prefetch's own first pass is what advances the store — and the moment you declined is stamped on the state mount, so a postponement is legible to the session it postponed rather than lost.
...
Viper is configured with `SetEnvPrefix("TOOLBOX")` + `AutomaticEnv`, which resolves a `TOOLBOX_*` variable only for a key viper already knows — so every key below is seeded or bound explicitly, and that seeding is what makes the override resolve at all:
...
| `TOOLBOX_PEER_MESSAGING` | `peer_messaging` |

For these keys the `TOOLBOX_*` env var sits at the top of the [loading order](#loading-order): it overrides every file layer (`--config`, project, global), and only the built-in default is below it. The set is `config.EnvBoundKeys` — every other `TOOLBOX_*` variable is ignored, silently, at unmarshal.
