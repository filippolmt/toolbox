...
**Symptom:** `herdr integration status` reports an integration you had installed as `not installed` after a `toolbox stop`, or as outdated against the herdr the image now ships.

**Cause:** herdr installs its integrations into each *agent's* own config directory, not under `~/.config/herdr` — so the `herdr` and `herdr-state` binds do not cover them, and survival depends entirely on whether that agent's directory is itself a mount. `claude` (`~/.claude/hooks/`), `codex` (`~/.codex/`) and `pi` (`~/.pi/agent/extensions/`) are bundled tools with a row in `internal/catalog`, so they land on the `~/.toolbox/.claude`, `~/.toolbox/.codex` and `~/.toolbox/.pi` binds and persist. A user-installed agent does not: `opencode`, for instance, is an npm global whose binary survives through `~/.npm-global`, while the integration herdr drops into `~/.config/opencode/plugins/` — like everything else it writes outside the mounts — is gone after `toolbox stop`. Persistence cuts both ways, and that is precisely why the bundled case is automated: because those registrations outlive a recreate, they would otherwise stay pinned to the herdr that installed them. `internal/build/assets/init.d/61-herdr.sh` rewrites all three on **every container start**, so the binary in the container is always the authority for its own hook.

**Fix:** for a bundled agent, `toolbox stop` and open a new shell — the recreate re-runs the install against the herdr the new image ships. A stale registration that survives that is a real failure, and the script says so on stderr at boot; `~/.toolbox/state/init/61-herdr.log` has the detail, and `herdr integration install <name>` re-runs it by hand. For a user-installed agent there is nothing to automate against — re-run the install after every recreate, or, better, move its config directory onto a mount so it stops vanishing: see [persisting a user-installed tool's state](mounts.md#persisting-a-user-installed-tools-state). `herdr integration status` lists the state of all of them.
