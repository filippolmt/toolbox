...
- Daemon-side: `playSound` writes the bytes to a temp file it names itself and spawns `afplay` (macOS) or the first installed player of the same chain (Linux) **detached**, answering `200` immediately. Blocking would queue two completions moments apart; the cost is that a failing player cannot be reported back **to the shim** — the request is answered before the first sample plays. It is reported into the daemon log instead: the goroutine that reaps the player logs its exit status and how long it lived, next to the handler's own `sound: ok`, and what the player wrote to `stderr` is read back onto that same line rather than left in `/dev/null` — an exit status is worth little without the sentence that explains it. That lifetime is the diagnostic for a *cut* chime — a player that lived as long as the MP3 puts the truncation past the player, in the host's output device, while one gone in milliseconds puts it here. A spawned player gets a deadline (`soundTimeout`), and the reaping goroutine removes the temp file.
- **One chime at a time**: a chime is seconds of audio, herdr coalesces only pane states far closer together than that, and two players on one output device replay the same MP3 out of phase — heard as a chime that garbles or cuts, not as two chimes. So a `/sound` request that arrives while a player is still running is answered `200` and **dropped**, with `sound: skipped` in the log. Dropping the later chime beats truncating the one already playing: overlapping chimes are indistinguishable to the listener anyway.
