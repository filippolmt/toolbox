...
- PR CI (`.github/workflows/ci.yml`) runs `go test` + `golangci-lint` (Go changes only — `**.md` / `docs/**` are path-ignored); the Docker smoke-test runs in `.github/workflows/docker-ci.yml` on image-affecting changes. Run `make go-check` locally to mirror the Go CI in one pass, or `make go-test` / `make go-lint` individually for faster feedback.
