...
- **Orphaned Sibling cleanup (`toolbox container`)**: acts on containers toolbox did NOT create — glossary: [Orphaned Sibling](../../CONTEXT.md#orphaned-sibling), rationale: [ADR 0004](../../docs/adr/0004-orphaned-sibling-cleanup.md). **One criterion, three consumers**: `container.Siblings` is the only place the target set is computed, and shell completion, `stop --all` and `prune` all read it — `BulkSiblings` is the *only* filter on top (drops `Proximo`, returns it as `skipped` so `cmd/container.go::selectTargets` can announce the skip; a target the TAB offered and the sweep passed over must never be silently absent). Identification is **positive**: Compose labels (`com.docker.compose.project` + `…project.working_dir` as the collision discriminator), the proximo stack told apart by `proximo.RoleLabel` — **not** `proximo.HostsLabel`, which marks *routed* app containers, the opposite population. `sessionplan.IsToolboxContainerName` is the refusal gate: a toolbox container is never a target and naming one errors towards `toolbox stop`. Removal is **not** `teardown.StopOne` (that is the policy for a container toolbox owns, lossless because state lives on host binds); grace is `siblingStopGrace` (10s, a test stack has a DB in it) and never `Force`. Networks go with `rm`/`prune`, volumes only behind `--volumes`, and the network/volume lookups **must** carry `projectFilter` — an unscoped label filter would reach every network on the host. Tests: `internal/container/siblings_test.go` (criterion, ambiguity, filter scoping), `cmd/container_test.go` (`TestSelectTargetsBulkIsTheCriterionMinusProximo` pins the shared set, plus usage guards and every verb against `fakeDocker`) — the verbs reach the daemon before deciding anything, so they go through the `newDockerClient` package var, never `container.NewClient` directly, or they become unreachable without a live daemon. This is the repo's first dynamic cobra completion: `completeSiblings` returns **zero suggestions and no error** when the daemon is unreachable — cobra writes whatever it emits into the user's prompt. → [toolbox container](../../docs/commands.md#toolbox-container)
