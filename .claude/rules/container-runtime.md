...
- **Session reload**: `toolbox-reload` (a zsh function in `zshrc.sh`, never a `bin/` script — a child cannot make its parent shell exit) writes `$PWD` into the [Reload Marker](../../CONTEXT.md#reload-marker) and exits; `container.Shell` reads and deletes it where `execShellFn` returns and hands `cmd` a typed `*reload.From`. **`cmd` does the `syscall.Exec` (`execSelf` var), never `container`** — and `Shell`'s teardown defer is suppressed by its own named `rl` result, because destroying there voids the verify-before-destroy gate. Order is **re-exec → Refresh + Ensure → teardown → create**: a reload with no usable image is a no-op, not a spent session. `TOOLBOX_RELOAD_MARKER` (host→container, presence = capability, value = path) and `TOOLBOX_RELOAD_FROM` (host→host across the exec, unset before any container env is built) share a prefix and point opposite ways — do not merge them. Every payload field degrades to a zero value **except `container`**, which is a hard error. The reload's teardown is unconditional and waits for the removal (`container.reloadTeardown`), never `teardown.OnShellExit`. The `syscall.Exec` line itself is an **accepted, permanently uncovered gap** — tests assert argv and payload, nothing runs the call. Pinned by `TestShellReloadSuppressesTeardown`, `TestShellReloadVerifiesBeforeItDestroys`, `TestShellReloadWithNoUsableImageDestroysNothing`, `TestShellReloadKillsThroughAnAttachedSibling`, `TestReloadMarkerContract`, `TestExecReloadCarriesTheHandover`, and the real-daemon `TestReloadReplacesTheContainer` (`dockergate`). → [session-reload-teardown](../../docs/internals/container-lifecycle.md#session-reload-teardown)
