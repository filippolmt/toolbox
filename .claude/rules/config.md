...
- Adding a key is one `configui.keyDescriptors` row (`descriptor.go`) plus that test case, never a new switch; `TestKeyDescriptorsCoverEveryKey` fails on a missing row instead of letting it surface as a blank TUI line. The row's `kind` is dispatched on **once**, in `editorSeeds` (one seed per editor kind, beside the kinds): `openEditor` looks its row's kind up there rather than switching on it again — a new editor kind is a seed there, never a branch in the tea half.
- The per-key facts that are NOT presentation are asked of their owner: `configedit.PerEntryKey` (which keys are attributed per entry) and `configedit.FileValues` (what one layer's file itself sets, per key — the deprecated-alias fold included, so `configui` neither parses a layer file nor names a `*yaml.Node`; the fold is the one `fillDefaultsBackstop` performs, guarded by `TestDeprecatedAliasesAreFoldedByTheLoadPath` on the load path and `TestFileValuesFoldsADeprecatedAliasIntoItsLiveKey` on the reader). `configui.scopeStates` keeps only the rendering: a counted key shows the file's entry count, everything else the file's scalar, and a descriptor row names the nested field its entries live under (`scopeEntries`) instead of carrying a node counter.
