...
- Adding a key is one `configui.keyDescriptors` row (`descriptor.go`) plus that test case, never a new switch; `TestKeyDescriptorsCoverEveryKey` fails on a missing row instead of letting it surface as a blank TUI line. A row declares *which* editor kind a key gets; what that kind opens with is `editorSeeds`, in `model.go` beside the kinds — the tea half, because the seed builds bubbles widgets and the adapter half has no business holding a `textinput`. `openEditor` looks its row's kind up there rather than switching on it again. A kind is still named in three places (its seed, its `renderEditor` case, its `updateEditing` case), and only the seed table is indexed by kind, so `TestEveryEditorKindIsSeededDrawnAndDriven` is the guard: it opens every kind, requires a pane that draws its own footer, and requires the kind's own key to change what is drawn. A new kind fails its completeness check until all three exist.
