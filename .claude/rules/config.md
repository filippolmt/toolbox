...
- **One row per key**: a config key's doc, human default, `config example` prose, render shape (`Kind`), editor kind (`Editor`), typed readers, validator, fail-fast scalar verdict and effective fallback are declared **once**, in `config.Keys()` (`internal/config/keys.go`) — [Key Row](../../CONTEXT.md#key-row). Adding or changing a key edits that row; the four surfaces (`config show`, `config ui`, `config example`, the validation tail) read it and never restate it. `TestEveryKeyHasACompleteRow` is the single guard — a row per `SchemaKeys()` entry, carrying what its `Kind` promises — so a surface's own tests assert its **output**, never that a row exists. `Keys()` order is Config declaration order, and that is the order all three surfaces present keys in (`TestSchemaKeys` pins it).
...
`config show`'s resolved-YAML renderer lives in `internal/configrender` (`Resolved`/`ResolvedWithOrigin`, peer of `internal/configexample` which renders the annotated *template*); `cmd/config.go` is flag parsing + dispatch only. Default output is byte-for-byte frozen (golden test) — annotations only behind `--origin`. It walks `config.Keys()` and shapes each key by its row's `Kind`, so a new key of an existing shape renders with no edit here; the three block keys (`worktree`, `shells`, `mounts`) carry entries too structured for a generic shape and keep their own writer, and an unhandled `Kind` is a returned error, never a missing line. It stays hand-rolled rather than marshalled through `yaml.v3` because the document is not a serialisation of `Config`: unset scalars render their fallback, a nil tri-state renders `auto`, an empty scalar renders `""`, and `--origin` appends the provenance label as bare trailing text — not a YAML comment, so no marshaller emits it. Scalar fallbacks (the effective value of an unset `shell`/`agent`/`pull`) come from the one `config.EffectiveValue(cfg, key)` seam, which reads the row's `Effective` — shared by the renderer, `configui.displayValue`, and `cmd/worktree.go`'s agent resolution so they can't drift; `TestRendererParity` guards the three.
...
Fail-fast on a bad flag value goes through the one `config.ValidateKey(key, value)` rule (keeps the usage-error exit code), never a `cmd`-private validator table or a direct `config.Validate*` call from a write path. It reads the key row's `Scalar` verdict — the same validator the row's `Validate` runs over the resolved Config — so the two cannot be written twice and drift: `TestEveryKeyHasACompleteRow` requires a scalar row to carry one (and a toggle row not to), and `TestValidateKeyAgreesWithTheValidationTail` holds its verdict to the load path's, word for word.

### `config ui`

`internal/configui` (bubbletea) is a thin presentation layer only — it resolves via `config.Plan`/`configedit.Compute` and writes through `configedit.ApplyChecked`. Config domain logic stays out. The tea half is not thin over the adapter funcs, though, and pretending it was is what left its decisions untested: **the tea half is tested through `Update` and `View`**, the seam bubbletea drives — a key press in, the rendered screen out (`tea_test.go`'s `press`/`screen`). A decision reachable only by calling a private method on a hand-built `Model` is a decision the running program cannot reach either, so a new one is encoded as the keystroke that triggers it, and the editor state it acts on is reached by pressing keys, never by assigning to `m.ed`.

**A message the status carries is asserted with `wantInStatus`, never `wantOnScreen`.** The view always draws a keybar naming `tab`, a detail pane naming the scope and the `TOOLBOX_*` var, and an inherit line naming the origin layer — so a whole-screen assertion on any of those passes whether the decision fired or not, which is the defect #903 was filed about. `statusArea` renders the model twice, once with the status cleared, and returns only what the status put there. `wantOnScreen` is for pane content. A new assertion is worth exactly what it excludes: break the behaviour it names and watch it fail before keeping it.
...
- Adding a key is its [Key Row](../../CONTEXT.md#key-row) — which carries the editor kind and the typed reader that seeds it — plus, when the key needs an option set or a writer, its `configui.keyDescriptors` entry (`descriptor.go`) and its `TestPreviewMatchesWriterForEveryEditableKey` case; never a new switch. Nothing demands a descriptor row: the behavioural sweeps (`TestEveryKeyDisplaysSomething`, `TestEveryEditableKeyOpensAnEditor`, `TestEveryOpenEditorHasAPendingMutation`) name the key that would surface as a blank TUI line or a runtime status message. One option set cannot move to the row — the default mount names come from `mountplan`, which imports `config` — so every option set stays here, together. What a kind *opens with* is `editorSeeds`, in `model.go` beside the kinds — the tea half, because the seed builds bubbles widgets and the adapter half has no business holding a `textinput`. `openEditor` looks the seed up by the kind the row declares rather than switching on it again. A kind is still named in three places (its seed, its `renderEditor` case, its `updateEditing` case), and only the seed table is indexed by kind, so `TestEveryEditorKindIsSeededDrawnAndDriven` is the guard: it opens every kind, requires a pane that draws its own footer, and requires the kind's own key to change what is drawn.
- The per-key facts that are NOT presentation are asked of their owner: `configedit.PerEntryKey` (which keys are attributed per entry) and `configedit.FileValues` (what one layer's file itself sets, per key — so `configui` neither parses a layer file nor names a `*yaml.Node`).
- **The deprecated-alias fold is written once**, as `config.FoldDeprecatedAliases`: it owns the pair table (`DeprecatedAliases`) and the precedence (live wins, the alias is a backstop). Both carriers call it — `fillDefaultsBackstop` on the decoded `*Config`, `configedit.FileValues` on a parsed document — and pass only their own answer to "does this carrier set the key", which is the irreducible difference: a nil tri-state pointer for the load path, an unwritten key for the reader. Never write the loop or the precedence a third time; that is what made `configui` describe its copy as folding "the same way `config.Merge` does". Guarded by `TestDeprecatedAliasesAreFoldedByTheLoadPath` (value equality, not "non-zero" — a live key with a default satisfies that on its own) and `TestFileValuesFoldsADeprecatedAliasIntoItsLiveKey` / `TestFileValuesPrefersTheLiveKeyOverItsAlias`. `configui.scopeStates` keeps only the rendering: a counted key shows the file's entry count, everything else the file's scalar, and a descriptor row names the nested field its entries live under (`scopeEntries`) instead of carrying a node counter.
- Test placement follows the files, which say what they hold: a new mutator's semantics go in `configedit/mutate_test.go` (pure, on bytes — that is where a CLI writer's node work belongs too, now that it is a mutator); `configedit/apply_test.go` owns the write pipeline (comment preservation, document bootstrap, the byte-equal short-circuit, the returned `existed` bit, rejection-without-writing, layer placement); `configedit/read_test.go` the single-file readers, `FileValues` included; `configui`'s own writer tests cover only that its saves reach that seam — with one carve-out: the reset's per-key artefact reconciliation (today the SDD `.gitignore` fences) is a decision only `configui` makes, so its scope asymmetry is asserted there, on disk, in `model_test.go` — driven by the `r` key press, like every other decision in that file.
