...
if command -v rtk >/dev/null 2>&1; then
    # Pre-seed ~/.config/rtk/config.toml on first start with tee disabled.
    # The default config has [tee] enabled = true with mode = "failures", which
    # writes failed-command stdout/stderr to disk (up to 20 files × 1MB each)
    # under ~/.local/share/rtk. Inside a toolbox shell those outputs often
    # include `gh auth status`, `aws sts ...`, `curl -H Authorization:` —
    # strings that can carry tokens or PII. Disabling tee removes a leakage
    # surface; debug value is low because the container is ephemeral.
    #
    # The seed is partial on purpose — rtk merges missing keys from its
    # built-in defaults at runtime. Caveat: a few rtk subcommands rewrite
    # the whole file (notably `rtk telemetry enable/disable`), which resets
    # [tee] back to enabled = true. That trade-off is acceptable: a user
    # who is consciously toggling rtk telemetry can re-edit two lines, and
    # we avoid stomping on their other custom edits on every shell start.
    # The file persists via the `rtk` bind-mount, so manual edits survive.
