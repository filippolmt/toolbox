...
//
// StepsFor, when non-nil, supersedes InstallSteps. Used by skills whose
// install surface depends on which catalog tools (e.g. claude, codex) the
// workspace has enabled — openspec installs per-tool adapter files via
// `--tools=<csv>`, so the step list is computed from cfg.Tools at session
// build time. Returning nil from StepsFor skips the skill entirely (no
// env vars emitted, bootstrap silent), used when the intersection of the
// skill's dependent tools with cfg.Tools is empty.
//
...
type Skill struct {
	...
	BinName          string
	InstallSteps     [][]string
	StepsFor         func(tools map[string]bool) [][]string
...
var Skills = []Skill{
	...
	},
	{
		...
		StepsFor: func(tools map[string]bool) [][]string {
			...
			return [][]string{
				{"init", "--tools=" + strings.Join(enabled, ","), "--force"},
				{"update"},
			}
		},
		GitignoreEntries: []string{
