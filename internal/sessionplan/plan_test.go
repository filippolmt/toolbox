...
// TestPlanSDDEnvCarriesManifestForGSD locks in the manifest-driven contract:
// gsd is the canonical manifest-managed skill, so its env must include
// TOOLBOX_SDD_GSD_MANIFESTS (comma-separated workspace-relative manifest
// paths) and TOOLBOX_SDD_GSD_EXTRAS (newline-separated sibling gitignore
// entries) that the entrypoint regen function consumes.
func TestPlanSDDEnvCarriesManifestForGSD(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	workspace := filepath.Join(tmpHome, "ws")
	if err := mkdirAll(t, workspace); err != nil {
	...
	envByKey := indexEnv(plan.Env)
	manifests, ok := envByKey["TOOLBOX_SDD_GSD_MANIFESTS"]
	if !ok {
		t.Fatal("missing TOOLBOX_SDD_GSD_MANIFESTS for manifest-managed skill")
	}
	for _, want := range []string{".claude/gsd-file-manifest.json", ".codex/gsd-file-manifest.json"} {
		if !strings.Contains(manifests, want) {
			t.Errorf("TOOLBOX_SDD_GSD_MANIFESTS = %q, missing %q", manifests, want)
		}
	}
	if !strings.Contains(manifests, ",") {
		t.Errorf("TOOLBOX_SDD_GSD_MANIFESTS = %q, expected comma separator between paths", manifests)
	}

	extras, ok := envByKey["TOOLBOX_SDD_GSD_EXTRAS"]
	if !ok {
		t.Fatal("missing TOOLBOX_SDD_GSD_EXTRAS for manifest-managed skill")
	}
	for _, want := range []string{
		...
		if !strings.Contains(extras, want) {
			t.Errorf("TOOLBOX_SDD_GSD_EXTRAS missing %q (got %q)", want, extras)
		}
	}
	if !strings.Contains(extras, "\n") {
		t.Errorf("TOOLBOX_SDD_GSD_EXTRAS = %q, expected newline separator between entries", extras)
	}
}

// TestPlanSDDEnvOmitsManifestForStaticSkills asserts the inverse: skills
// that ship a static GitignoreEntries list (or none at all) must NOT emit
// the manifest/extras pair, since the entrypoint regen branch keys off the
// MANIFESTS variable being non-empty.
func TestPlanSDDEnvOmitsManifestForStaticSkills(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	workspace := filepath.Join(tmpHome, "ws")
	if err := mkdirAll(t, workspace); err != nil {
	...
	cfg := testConfig()
	cfg.SDD = map[string]bool{"bmad": true, "openspec": true}
	...
	for _, key := range []string{
		"TOOLBOX_SDD_BMAD_MANIFESTS",
		"TOOLBOX_SDD_BMAD_EXTRAS",
		"TOOLBOX_SDD_OPENSPEC_MANIFESTS",
		"TOOLBOX_SDD_OPENSPEC_EXTRAS",
	} {
		for _, e := range plan.Env {
			if strings.HasPrefix(e, key+"=") {
				t.Errorf("non-manifest-managed skill must not emit %q (got %q)", key, e)
			}
		}
	}
}
