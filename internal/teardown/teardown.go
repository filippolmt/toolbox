...
func StopOne(ctx context.Context, cli client.APIClient, name string, stopGrace int) error {
	...
	if cerrdefs.IsNotFound(stopErr) {
		ui.Warning(fmt.Sprintf("Container %s not found", name))
		return nil
	}
	if stopErr != nil {
	...
	ui.Success(fmt.Sprintf("Container %s stopped and removed", name))
	return nil
}
...
func OnShellExit(cli client.APIClient, name string) error {
	...
	if execsRunning(ctx, cli, inspect) {
		ui.Info(fmt.Sprintf("Container %s still has active sessions — leaving it running", name))
		return nil
	}
...
func killAutoRemove(ctx context.Context, cli client.APIClient, name string) error {
	...
	ui.Success(fmt.Sprintf("Container %s stopped (removing in background)", name))
	return nil
}
