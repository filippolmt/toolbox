...
- **rtk hook auto-wiring on shell start**: `entrypoint.sh` runs `rtk init -g` (Claude) and `rtk init -g --codex` (Codex) on every shell so the Bash-tool rewrite hook stays registered even after a settings reset or a fresh `~/.toolbox/.claude` bind-mount. Gated on `command -v claude` / `command -v codex` so opted-out tools never have rtk hooks injected. Idempotent; failures are non-fatal. Telemetry is killed image-wide via `RTK_TELEMETRY_DISABLED=1`; the entrypoint also pre-seeds `~/.config/rtk/config.toml` with `[tee] enabled = false` on first launch so failed-command stdout (which often carries auth tokens) is not written to disk.
- **Claude Code plugin auto-update is delegated to Claude Code itself.** The image sets `DISABLE_AUTOUPDATER=1` (block background CLI self-update — `/usr/local/lib/node_modules` is root-only) and `FORCE_AUTOUPDATE_PLUGINS=1` (the documented escape hatch from the [discover-plugins guide](https://code.claude.com/docs/en/discover-plugins#configure-auto-updates) that keeps plugin updates running even when the CLI auto-updater is disabled). When a plugin is refreshed Claude Code prompts for `/reload-plugins`. Bumping the CLI itself is a Dockerfile concern: `CLAUDE_CODE_VERSION` + image rebuild (Renovate-driven). `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` is intentionally NOT set: it was correlated with intermittent OAuth re-login prompts on long-lived shells, likely because the umbrella flag also throttles token-refresh requests. Privacy trade-off accepted.
