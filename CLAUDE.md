...
- `internal/container` — Docker SDK wrapper, split by concern: `lifecycle.go` (state machine: ensure / start / stop / cleanup, calls `mountplan.Plan` once), `attach.go` (TTY raw mode + signal forwarding), `names.go` (`ContainerNameFor`: `toolbox-<basename>-<hash8>`), `workspace.go` (just `shellEnv`: TOOLBOX_HOST_WORKSPACE + PWD env-var injection — the bind + mirror logic moved to `mountplan`), `ports.go` (`--publish` parsing, defaulting host IP to `127.0.0.1`, and the wanted-vs-actual mismatch warning). Calls `client.WithAPIVersionNegotiation()` so the SDK pin in `go.mod` need not match the runtime CLI version.
