...
- **rtk skipped on arm64 by design** (Layer 13c): upstream only ships `aarch64-unknown-linux-gnu`, linked against GLIBC 2.39, but the base image (`node:24-bookworm-slim`) ships GLIBC 2.36 — `rtk --version` aborts with `'GLIBC_2.39' not found`. amd64 still installs the static MUSL build. Re-enable when upstream publishes `aarch64-unknown-linux-musl` or when the base image moves to Debian trixie. Smoke-test SKIPs rtk automatically; entrypoint's `command -v rtk` gate keeps the hook-init step silent on arm64.
