...
- **Auth isolation under `~/.toolbox/`**: every credential path the container sees lives under `~/.toolbox/` on the host (`.claude`, `state`, `gh`, `glab`, `rtk/{config,data}`, `cf/{auth,config}`, …) or is a symlink to the host's real file (`ssh`, `gitconfig`). See `internal/config/config.go` `DefaultMounts()`. `~/.secrets` is intentionally NOT mounted. rtk and cf are the two tools whose state spans two binds because upstream splits config across non-XDG paths and exposes no env override: rtk uses `~/.config/rtk` (config) + `~/.local/share/rtk` (analytics/tee dumps), while cf uses `~/.cf/config.toml` (OAuth tokens) + `~/.config/cf/config.json` (context defaults, completion marker). In both cases the bind sources are nested under a single `~/.toolbox/<tool>/` root so the host layout stays flat.
