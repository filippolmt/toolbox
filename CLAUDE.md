...
- **PID 1 is `entrypoint.sh` → `sleep infinity`** (the default `CMD`). The user's interactive shell runs via `docker exec`, not as PID 1, so the lack of a TTY-owning shell at PID 1 is intentional: an interactive zsh/bash there would ignore `SIGTERM` and force `docker stop` to fall back to `SIGKILL` after the full grace period. `sleep` exits instantly on `SIGTERM`, so `toolbox shell` exit is sub-second. If you change CMD back to a shell, also restore an init wrapper (e.g. tini) — otherwise the smoke test's "Signal handling check" will fail and users will see the old 10s exit hang.
