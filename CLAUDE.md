...
- `internal/container` — Docker SDK wrapper, two files by Module/Adapter split: `lifecycle.go` is the orchestration Module owning `Shell`/`Stop`/`StopAll`/`ResolveShellCmd`/`NewClient` plus the unexported helpers, sectioned into `Naming` (container-name hashing under the `toolbox-` prefix), `Workspace Env` (`TOOLBOX_HOST_WORKSPACE` + `PWD` injection), `Port Bindings` (`--publish` parsing, default host IP `127.0.0.1`, wanted-vs-actual mismatch warning), `Lifecycle` (state machine + image ensure + create/start), and `Cleanup helpers` (stop/remove + active-exec detection); calls `mountplan.Plan` once per `Shell`. `attach.go` stays a separate Adapter — TTY raw mode + signal forwarding kept off the Docker-SDK orchestration path. Both files call `client.WithAPIVersionNegotiation()` so the SDK pin in `go.mod` need not match the runtime CLI version.
