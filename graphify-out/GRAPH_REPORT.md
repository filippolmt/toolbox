...
## Corpus Check
- 114 files · ~73,021 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1230 nodes · 1682 edges · 136 communities (92 shown, 44 thin omitted)
- Extraction: 81% EXTRACTED · 19% INFERRED · 0% AMBIGUOUS · INFERRED: 324 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fb94a7ff`
...
8. `testPlan()` - 17 edges
9. `runSDDInit()` - 16 edges
10. `Go CLI Best Practices` - 16 edges
...
Cohesion: 0.31
Nodes (13): Image, MergedSessionPlan, ContainerNameFor(), hashWorkspace(), rationale: Default host IP 127.0.0.1 (not 0.0.0.0) so OAuth callbacks stay loopback-only, Merge(), func Merge(cfg, workspace, ports, cliVersion) (*MergedSessionPlan, error), normalizeWorkspace() (+5 more)
...
Cohesion: 0.16
Nodes (9): Defaults(), IsDefault(), TestCanonicalEncodingDeterministic(), TestCanonicalEncodingIsNeutralToOptionalFieldPopulation(), TestIsDefaultMatchesLegacy(), WriteCanonical(), WriteCanonicalEntries(), Entry (+1 more)

### Community 4 - "Config Init Tests"
Cohesion: 0.08
Nodes (38): resetCmdState(), TestInitConfigAppliesDefaults(), TestInitConfigExplicitFileIsRead(), TestInitConfigProjectFileFromCWD(), TestInitConfigProjectFileStopsAtHome(), TestInitConfigProjectFileWalksUpFromSubdir(), TestPlanGlobalUnreadableIsBestEffort(), TestPlanWalksUpFromSubdir() (+30 more)
...
Cohesion: 0.10
Nodes (34): DefaultTools(), Shell(), captureStderr(), stubExecShell(), testConfig(), testPlan(), testPlanWithCfg(), TestShellAutoBuildsCustomImage() (+26 more)
...
Cohesion: 0.09
Nodes (17): TestUsageArgsWraps(), cmd/build.go — toolbox build subcommand, cmd/completion.go — toolbox completion subcommand, cmd/config.go — toolbox config show/example, TestWriteResolvedConfigDeterministic(), TestWriteResolvedConfigEmptyMounts(), TestWriteResolvedConfigNilConfigErrors(), writeResolvedConfig() (+9 more)
...
Cohesion: 0.15
Nodes (26): runInit(), chdirTemp(), TestInitForceOverwrites(), TestInitRefusesOverwriteWithoutForce(), TestInitWritesAnnotatedYAML(), usageError — flag/arg error wrapper for exit code 2, gitignoreFenceEnd(), gitignoreFenceStart() (+18 more)
...
Cohesion: 0.17
Nodes (11): enabledPlugins, andrej-karpathy-skills@karpathy-skills, claude-code-setup@claude-plugins-official, code-review@claude-plugins-official, code-simplifier@claude-plugins-official, commit-commands@claude-plugins-official, gopls-lsp@claude-plugins-official, pr-review-toolkit@claude-plugins-official (+3 more)

### Community 23 - "Container Mocks"
Cohesion: 0.06
Nodes (34): CLI commands, code:bash (brew install filippolmt/tap/toolbox), code:yaml (mounts:), code:bash (# Forward host port 7171 to the same port in the container.), code:bash (toolbox shell -p 8976:8976), code:bash (brew upgrade toolbox), code:bash (docker pull ghcr.io/filippolmt/toolbox:latest), code:bash (go install github.com/filippolmt/toolbox@latest) (+26 more)
...
Cohesion: 0.17
Nodes (12): const EnvEnabled = TOOLBOX_SDD_ENABLED, const EnvSkillPrefix = TOOLBOX_SDD_, const EnvWorkspaceHash = TOOLBOX_SDD_WORKSPACE_HASH, const StepSeparator = ;, Keys(), Lookup(), SkillEnvKey(), Skill (+4 more)
...
Cohesion: 0.07
Nodes (28): Auth isolation under `~/.toolbox/`, Catalog entry → image hash, `cf` Cloudflare CLI skill auto-install, Claude Code env-var matrix, code:block1 (# >>> sdd-managed/<name> (toolbox)), Codex nested sandbox, Container lifecycle, Docker CLI checksum (+20 more)

### Community 92 - "Community 92"
Cohesion: 0.17
Nodes (12): rationale: Versions Renovate-bumped via customManager keyed on NpmPackage, Skill entry: bmad (bmad-method), Skill.BinName, Skill.GitignoreEntries, Skill.InstallSteps, Skill.Key, Skill.NpmPackage, Skill.RequiresMarker (+4 more)
...
Cohesion: 0.08
Nodes (23): Always Bind Flags to Viper, Argument Validation, code:block1 (myapp/), code:yaml (port: 8080), Common Mistakes, Configuration with Viper, Example Config File (.myapp.yaml), Exit Codes (+15 more)

### Community 95 - "Community 95"
Cohesion: 0.09
Nodes (22): Benchmarks, code:block1 (internal/), code:block10 (test/), code:block11 (internal/), code:bash (go test ./...                    # Run all tests), code:go (package handler  // Same package, can access unexported), code:go (package handler_test  // Different package, only exported AP), code:block4 (pkg/) (+14 more)
...
Cohesion: 0.11
Nodes (18): code:block1 (project/), code:block2 (my-tool/), code:block3 (my-library/), code:block4 (cmd/), code:block5 (cmd/), code:bash (go build ./cmd/...        # Build all main packages), code:block7 (myproject/), code:block8 (myproject/) (+10 more)
...
Cohesion: 0.12
Nodes (15): 12-Factor App, Architecture Decision: Ask First, code:go (// ✅ Good), Dependency Injection: Ask Next, Directory Layout, Essential Configuration Files, Go Project Layout, Go Workspaces (+7 more)

### Community 101 - "Community 101"
Cohesion: 0.14
Nodes (11): catalog.Entries (canonical tool table), dockerfile bijection test, init.d bijection test, internal/sdd Skill registry (conceptual), `toolbox sdd init gsd` (replaces legacy hook), Config.SDD (top-level, hash-neutral), config.Config struct, Render() (+3 more)

### Community 102 - "Community 102"
Cohesion: 0.17
Nodes (10): Docker CLI Checksum policy, Docker publish to GHCR on main, rtk hook auto-wiring + privacy lockdown, Skill discovery paths diverge (Claude vs Codex), Toolbox (containerised dev environment), Two Docker version streams, /verify skill (mirrors CI), Commits & PRs (+2 more)
...
Cohesion: 0.36
Nodes (9): Keys(), ValidateMountsRoot(), applyValidationTail(), dryParseYAML(), fillToolDefaultsBackstop(), Merge(), Plan(), seedToolDefaults() (+1 more)
...
Cohesion: 0.25
Nodes (6): BuildArg(), TestBuildArgLookup(), TestKnownToolsIncludesZsh(), TestIsDefaultTools(), TestToolBuildArgGo(), IsDefaultTools()
...
Cohesion: 0.33
Nodes (5): Application Configuration with Cobra + Viper, code:block1 (myapp/), code:go (// cmd/myapp/config.go), Config Struct, Where Config Lives
...
Cohesion: 0.33
Nodes (3): cmd package (cobra root), Release flow (v* tag triggers GoReleaser + Homebrew), internal/version package

### Community 125 - "Community 125"
Cohesion: 0.33
Nodes (5): Config, Load(), TestLoadSmoke(), Mount, NamedShell
...
Cohesion: 0.67
Nodes (3): TestValidateShellAcceptsSupported(), TestValidateShellRejectsUnknown(), ValidateShell()

### Community 131 - "Community 131"
Cohesion: 0.50
Nodes (3): Answer, Q: Why does Shell() connect Container Lifecycle Tests to CLI Init + Lifecycle, Run Plan State Machine, Session Plan Core, Image Plan Ensure?, Source Nodes
...
Cohesion: 0.53
Nodes (4): TestKeysMatchesSkills(), TestLookupReturnsRegisteredSkills(), TestRenovateRegexMatchesEverySDDPin(), TestSkillEnvKey()

### Community 134 - "Community 134"
Cohesion: 0.40
Nodes (5): concept: SDD env contract between sessionplan.sddEnv and entrypoint.sh, rationale: Skills excluded from internal/catalog to keep image hash neutral (per-repo enablement must not force toolbox:local-<hash> rebuild), package sdd (registry), concept: Three-step add-skill contract (row + Renovate entry + optional gitignore), package sessionplan (plan)

## Knowledge Gaps
- **385 isolated node(s):** `version`, `source`, `sourceType`, `skillPath`, `computedHash` (+380 more)
...
- **Why does `Shell()` connect `Container Lifecycle` to `CLI Subcommands & Init`, `Community 135`, `Image Plan`, `Run Plan Pipeline`, `Community 86`, `Community 87`, `Community 88`?**
...
- **Why does `Lookup()` connect `Community 26` to `toolbox init Scaffold`, `Community 133`, `Community 92`?**
